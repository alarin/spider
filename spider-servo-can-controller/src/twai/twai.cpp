...
void TWAI::setup(gpio_num_t tx_pin, gpio_num_t rx_pin) {
...
void TWAI::sendAngle(uint8_t legn, uint8_t motorn, float angle) {
...
void TWAI::requestStatus(uint8_t legn, uint8_t motorn) {
    twai_message_t tx_msg;
...
bool TWAI::receive(motor_status_t *motor_status, uint32_t ticks) {
    twai_message_t rx_msg;
    if (twai_receive(&rx_msg, pdMS_TO_TICKS(ticks)) == ESP_OK) {
        ESP_LOGI(TAG, "Received CAN ID: 0x%lx", rx_msg.identifier);
        ESP_LOG_BUFFER_HEX(TAG, rx_msg.data, rx_msg.data_length_code);

        if (rx_msg.identifier & (MOTOR_FEEDBACK << 6)) {
            memcpy(motor_status, &rx_msg.data, sizeof(motor_status_t)); 
            ESP_LOGI(TAG, "Detected motor status state %d, angle %f", motor_status->state, motor_status->angle);
            return true;
        }
        return false;
    }
